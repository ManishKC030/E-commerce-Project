...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($status, ['pending', 'confirmed', 'shipped', 'delivered', 'cancelled'])) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Failed to update order status.'); window.location.href = 'view_orders.php';</script>";
        }
    } else {
        echo "<script>alert('Invalid status selected.'); window.location.href = 'view_orders.php';</script>";
    }
} else {
